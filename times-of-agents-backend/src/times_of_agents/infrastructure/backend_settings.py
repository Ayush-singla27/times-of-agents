...
class BackendSettings:
    ...
    def from_config_file(cls, config_file: Path | None = None) -> BackendSettings:
        ...
        settings = cls(
            ...
            insights_per_run=_parse_int(
                payload.get("insights_per_run"),
                defaults.insights_per_run,
                "insights_per_run",
            ),
            ...
            host=_parse_str(payload.get("host"), defaults.host),
            port=_parse_int(payload.get("port"), defaults.port, "port"),
        )
        settings._validate()
        return settings
    ...
    def _validate(self) -> None:
        ...
        if self.insights_per_run < 0:
            raise ValueError("insights_per_run must be non-negative")
