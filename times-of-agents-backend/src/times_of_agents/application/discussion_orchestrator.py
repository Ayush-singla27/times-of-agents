...
from times_of_agents.application.agent_builder import build_crewai_agent
from times_of_agents.application.agent_prompts import build_system_prompt
from times_of_agents.application.discussion_tasks import (
...
def _log_usage_summary(
    ...
    logger.info(
        "Discussion token usage%s: input=%s, output=%s, total=%s, calls=%s",
        estimate_note,
    ...
    if input_rate == 0.0 and output_rate == 0.0:
        logger.info(
            "Discussion token cost: $0.000000 (set token_input_cost_per_1k_usd and "
            "token_output_cost_per_1k_usd in backend config to enable cost estimation)"
        )
    else:
        logger.info(
            "Discussion token cost: $%.6f (input $%.6f + output $%.6f, rates: $%.4f/$%.4f per 1K)",
...
def run_discussion(
    ...
    for round_index in range(1, rounds + 1):
        ...
        for position, cfg in enumerate(ordered_configs):
            ...
            is_speaking_estimate = speaking_usage.call_count == 0
            if is_speaking_estimate:
                ...
                used_estimate = True
            log_agent_usage(
                cfg.identity.name, f"R{round_index} speak",
                speaking_usage, input_rate, output_rate, estimated=is_speaking_estimate,
            )
            ...
            interjections_on_this_message = 0
            for idx in range(position + 1, len(ordered_configs)):
                if interjections_on_this_message >= 2:
                    break
                ...
                is_interject_estimate = interjection_usage.call_count == 0
                if is_interject_estimate:
                    ...
                    used_estimate = True
                log_agent_usage(
                    other_cfg.identity.name, f"R{round_index} interject",
                    interjection_usage, input_rate, output_rate, estimated=is_interject_estimate,
                )
                ...
                interjections_on_this_message += 1
                transcript.append(
                    Message(
                        round_index=round_index,
    ...
    _log_usage_summary(usage_summary, input_rate, output_rate, used_estimate)
